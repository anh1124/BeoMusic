...
//        );
        ...
        btnShuffle.setOnClickListener(v -> {
            isShuffle = !isShuffle;
            btnShuffle.setAlpha(isShuffle ? 1f : 0.5f);
        });
    }
    ...
    private void handleSongCompletion() {
        if (isRepeat)
        {
            loadSongFromList(currentPosition); // Phát lại bài hiện tại
        }
        else if (isShuffle) {
            Random random = new Random();
            currentPosition = random.nextInt(songList.size());
            Log.d("Current number", String.valueOf(currentPosition));
            loadSongFromList(currentPosition);
        } else {
            currentPosition++;
            if (currentPosition >= songList.size()) {
                currentPosition = 0; // Reset về đầu danh sách nếu hết
            }
            loadSongFromList(currentPosition);
        }
    }
